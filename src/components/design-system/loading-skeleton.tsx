  ...
  return (
    <div className={cn("space-y-3", className)}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={`sk-l-${String(i).padStart(3, '0')}`} className="flex items-center gap-3 rounded-lg p-3">
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={`sk-s-${String(i).padStart(3, '0')}`} className="rounded-xl border border-border bg-card p-4 space-y-3">
